...
async function initStore() {
    const module = await Promise.resolve().then(() => __importStar(require('electron-store')));
...
async function executeTool(toolName, toolInput) {
    const cliPath = path.join(electron_1.app.getAppPath(), '..', 'dist', 'index.js');
    const cwd = path.join(electron_1.app.getAppPath(), '..');
...
async function sendToClaude(apiKey, model, userMessage, history) {
    try {
        const messages = [
            ...
            const toolResultMessages = [
                ...
                { role: 'assistant', content: data.content },
                {
                    ...
                    content: toolCalls.map(t => ({
                        type: 'tool_result',
                        tool_use_id: data.content.find((c) => c.type === 'tool_use' && c.name === t.name)?.id,
                        content: t.output
                    }))
                }
            ];
...
async function sendToGemini(apiKey, model, userMessage, history) {
    try {
...
electron_1.ipcMain.handle('send-chat-message', async (_event, message, history) => {
