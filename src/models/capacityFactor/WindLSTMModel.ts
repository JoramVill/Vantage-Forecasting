/**
 * WindLSTMModel - LSTM Neural Network for Wind Capacity Factor Forecasting
 *
 * This model uses Long Short-Term Memory (LSTM) neural networks to capture
 * temporal patterns in wind generation that traditional models miss.
 *
 * Key features:
 * - Sequence-based learning: Uses past N hours of data to predict current hour
 * - Weather features: windspeed, windgust, temperature, pressure, cloud cover
 * - Temporal features: hour of day, day of week (encoded)
...
export interface LSTMTrainingOptions {
  sequenceLength?: number;    // Number of past hours to use (default: 24)
  hiddenLayers?: number[];    // Hidden layer sizes (default: [32, 16])
...
export class WindLSTMModel {
  ...
  private metrics: LSTMModelMetrics | null = null;
  ...
  private static readonly FEATURE_NAMES = [
    'windspeed_100m',      // Primary wind speed
    'windgust',            // Wind gusts
    'gust_ratio',          // gustRatio = gust/speed
    'temperature',         // Air temperature (density proxy)
    'cloud_cover',         // Cloud coverage
    'hour_sin',            // Hour of day (sine encoded)
    'hour_cos',            // Hour of day (cosine encoded)
    'prev_cf',             // Previous capacity factor (if available)
  ];

  constructor(stationCode: string, options: Partial<LSTMTrainingOptions> = {}) {
    this.stationCode = stationCode;
    this.sequenceLength = options.sequenceLength ?? 24;
    this.featureCount = WindLSTMModel.FEATURE_NAMES.length;
  }
   ...
   */
  getMetrics(): LSTMModelMetrics | null {
    return this.metrics;
  }
  ...
  private normalizeFeatures(features: number[]): number[] {
    if (this.featureMeans.length === 0) {
      // If not trained, return raw features (will be normalized during training)
      return features;
    }
  ...
  private initializeNetwork(): void {
    ...
    // (Synaptic's LSTM is more complex to configure)
    // Using a deeper feedforward network as an approximation
    // that can learn temporal patterns through the flattened sequence input
    this.network = new Architect.Perceptron(
      inputSize,
      Math.floor(inputSize / 2),  // First hidden layer
      32,                          // Second hidden layer
  ...
  train(
    ...
    options: Partial<LSTMTrainingOptions> = {}
  ): LSTMModelMetrics {
    const startTime = Date.now();

    console.log(`\n  Training LSTM for ${this.stationCode}...`);
  ...
  private evaluate(sequences: SequenceData[]): { mape: number; mae: number } {
    ...
    for (const seq of sequences) {
      ...
      if (actual > 0.01) {  // Avoid division by near-zero
   ...
   */
  static fromJSON(json: any): WindLSTMModel {
    const model = new WindLSTMModel(json.stationCode, {
